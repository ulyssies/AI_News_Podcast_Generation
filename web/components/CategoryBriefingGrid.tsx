        ...
        {BRIEFING_CATEGORIES.map((cat) => {
          ...
          return (
            <button
              ...
              className={`group flex flex-col items-start text-left rounded-md border px-1.5 py-1.5 sm:px-2 sm:py-2 min-h-[4.625rem] sm:min-h-[4.875rem] transition-all duration-150 hover:brightness-[1.05] active:scale-[0.99] disabled:opacity-40 ${cat.accent}`}
            >
              <Icon
                className={`h-[19px] w-[19px] shrink-0 ${cat.iconGlow}`}
                strokeWidth={1.6}
                aria-hidden
              />
              <span className="mt-0.5 text-[9px] sm:text-[10px] font-semibold text-white leading-[1.2] line-clamp-2">
                {cat.label}
              </span>
              <span className="mt-0.5 text-[8px] sm:text-[9px] text-slate-400/85 leading-[1.25] line-clamp-2">
                {cat.description}
              </span>
              {busy && (
